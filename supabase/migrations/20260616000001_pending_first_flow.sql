...
--
-- 変更点:
--   Web からの新規予約は「仮確定 (pending)」で登録する（従来は confirmed）。
--   オーナーが管理画面で承認（confirmed へ変更）すると本確定になり、
--   アプリ側からお客様へ「ご予約確定」メールが送信される。
--
-- 備考:
--   - EXCLUDE 制約は pending / confirmed 両方を対象にしているため、
--     仮確定の時点で枠は塞がる（ダブルブッキングは起きない）。
--   - cancel_reservation / リマインダー cron も pending を対象に含むため
--     追加変更は不要。
--   - Stripe 事前決済は「入金 = 確定」とみなし、Webhook が confirmed に
--     更新する挙動を維持する。
-- ============================================================================
...
begin
  ...
  -- INSERT（仮確定。オーナー承認で confirmed へ）
  begin
    insert into public.reservations (
      ...
      v_total_price, p_payment_method, 'pending', 'web', p_notes
    )
